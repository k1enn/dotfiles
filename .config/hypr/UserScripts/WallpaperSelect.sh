...
if ! command -v bc &>/dev/null; then
  notify-send -i  "bc missing" "Install package bc first"
  exit 1
fi
  ...
  if [ -d "$sddm_simple" ]; then
    ...
    if yad --info --text="Set current wallpaper as SDDM background?\n\nNOTE: This only applies to SIMPLE SDDM v2 Theme" \
      ...
      if ! command -v "$terminal" &>/dev/null; then
        notify-send -i  "Missing $terminal" "Install $terminal to enable setting of wallpaper background"
        exit 1
      fi
...
apply_video_wallpaper() {
  ...
  if ! command -v mpvpaper &>/dev/null; then
    notify-send -i  "E-R-R-O-R" "mpvpaper not found"
    return 1
  fi
  kill_wallpaper_for_video
