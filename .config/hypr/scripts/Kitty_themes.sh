...
apply_kitty_theme_to_config() {
  ...
  if [ ! -f "$theme_file_path_to_apply" ]; then
    notify_user  "Error" "Theme file not found: $theme_name_to_apply.conf"
    return 1
  fi
...
if [ ! -d "$kitty_themes_DiR" ]; then
  notify_user  "E-R-R-O-R" "Kitty Themes directory not found: $kitty_themes_DiR"
  exit 1
fi

if [ ! -f "$rofi_theme_for_this_script" ]; then
  notify_user  "Rofi Config Missing" "Rofi theme for Kitty selector not found at: $rofi_theme_for_this_script."
  exit 1
fi
...
if [ ${#available_theme_names[@]} -eq 0 ]; then
  notify_user  "No Kitty Themes" "No .conf files found in $kitty_themes_DiR."
  exit 1
fi
...
while true; do
  ...
  if [ $rofi_exit_code -eq 0 ]; then
    ...
    notify_user  "Kitty Theme" "Selection cancelled. Reverting to original theme."
    echo "$original_kitty_config_content_backup" >"$kitty_config"
    for pid_kitty in $(pidof kitty); do if [ -n "$pid_kitty" ]; then kill -SIGUSR1 "$pid_kitty"; fi; done
    break
  elif [ $rofi_exit_code -eq 10 ]; then # This is the exit code for -kb-custom-1
    notify_user  "Kitty Theme Applied" "$theme_to_preview_now"
    break
  else
