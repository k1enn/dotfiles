...
if command -v paru &> /dev/null || command -v yay &> /dev/null; then
  ...
  notify-send -u low 'Fedora system' 'has been updated.'
elif command -v apt &> /dev/null; then
  # Debian-based (Debian, Ubuntu, etc.)
  kitty -T update sudo apt update && sudo apt upgrade -y
  notify-send -u low 'Debian/Ubuntu system' 'has been updated.'
elif command -v zypper &> /dev/null; then
  # openSUSE-based
  kitty -T update sudo zypper dup -y
  notify-send -u low 'openSUSE system' 'has been updated.'
else
  # Unsupported distro
