...
# Rofi command
rofi_command="rofi -show -dmenu -config $rofi_theme -theme-str $rofi_override"
  ...
  for pic_path in "${sorted_options[@]}"; do
    ...
    if [[ "$pic_name" =~ \.gif$ ]]; then
      ...
      if [[ ! -f "$cache_preview_image" ]]; then
        ...
        ffmpeg -v error -y -ss 00:00:01.000 -vframes 1 "$cache_preview_image"
      fi
  ...
  if [ -d "$sddm_simple" ] && [ -w "$sddm_simple/Backgrounds" ]; then
    ...
    if yad --info --text="Set current wallpaper as SDDM background?\n\nNOTE: This only applies to SIMPLE SDDM v2 Theme" \
      ...
      if ! command -v "$terminal" &>/dev/null; then
        notify-send "Missing $terminal" "Install $terminal to enable setting of wallpaper background"
        exit 1
      fi
