...
if [ -z "$local_version" ]; then
  notify-send "ERROR "!?!?!!"" "Unable to find KooL's dots version . exiting.... "
  exit 1
fi
    ...
    "action1")  
      if [ -d $KooL_Dots_DIR ]; then
      	if ! command -v kitty &> /dev/null; then
  			notify-send "E-R-R-O-R" "Kitty terminal not found. Please install Kitty terminal."
  			exit 1
		fi
        kitty -e bash -c "
          cd $KooL_Dots_DIR &&
          ...
          git pull &&
          ./copy.sh &&
		  notify-send -u critical 'Update Completed:' 'Kindly log out and relogin to take effect'
        "
