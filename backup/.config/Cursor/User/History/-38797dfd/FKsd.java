...
import com.example.elsa_speak_clone.database.entities.UserStats;
...
public class FirebaseDataSynchronizer {
                        ...
                        for (User user : userList) {
                            try {
                                ...
                                if (existingUser == null) {
                                    ...
                                    try {
                                        // Wait for the progress data to be fetched and imported
                                        boolean progressImported = restoreUserProgress(userId).get();
                                        if (progressImported) {
                                            Log.d(TAG, "Imported progress for user " + userId);
                                        } else {
                                            Log.d(TAG, "No progress found for user " + userId);
                                        }
                                    } catch (Exception e) {
                                        Log.e(TAG, "Error importing progress for user " + userId, e);
                                    }
                                    
                                    importCount++;
                                } else {
                                    // User exists - check if we should update progress
                                    int userId = user.getUserId();
                                    try {
                                        // Get user stats from Firebase
                                        UserStats stats = firebaseDbHelper.getUserStats(userId).get();
                                        if (stats != null) {
                                            // Check local progress
                                            UserProgress localProgress = database.userProgressDao().getUserProgressById(userId);
                                            if (localProgress == null || stats.getXp() > localProgress.getXp() || 
                                                stats.getStreak() > localProgress.getStreak()) {
                                                // Firebase has newer progress - update local
                                                updateUserProgressFromStats(userId, stats);
                                                Log.d(TAG, "Updated progress for existing user " + userId);
                                            }
                                        }
                                    } catch (Exception e) {
                                        Log.e(TAG, "Error checking progress for existing user " + userId, e);
                                    }
                                }
                            } catch (Exception e) {
    ...
    /**
     * Helper method to update user progress from stats
     */
    private void updateUserProgressFromStats(int userId, UserStats stats) throws Exception {
