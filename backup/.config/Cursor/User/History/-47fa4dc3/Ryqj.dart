...
class CryptoCandleStickChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final List<CandleData> candleData = _generateSampleCandleData(); // TODO: Using API to change this
  ...
  String _getDateForSpot(int index) {
    if (index >= 0 && index < _getHistoricalData().length) {
      final date = _getHistoricalData()[index].time;
...
class CryptoCharts extends StatefulWidget {
  final CryptoCurrency crypto;

  const CryptoCharts({super.key, required this.crypto});

  @override
  State<CryptoCharts> createState() => _CryptoChartsState();
}

class _CryptoChartsState extends State<CryptoCharts> {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        CryptoCandleStickChart(crypto: widget.crypto),
        Container(
          ...
          child: Row(
            ...
            children: [
              for (final period in ['24h', '7d', '1m', '1y'])
                Padding(
