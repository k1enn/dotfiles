        ...
        while True:
            ...
            if self.add_mode or self.edit_mode:
                ...
                try:
                    ...
                    active_field_color = curses.A_REVERSE | curses.A_BOLD
                    inactive_field_color = curses.A_NORMAL
                    
                    # Draw name field with proper formatting
                    name_label = f"{mode} Task Name: "
                    stdscr.addstr(height - 4, 2, name_label)
                    name_field_attr = active_field_color if self.name_input_active else inactive_field_color
                    stdscr.addstr(height - 4, 2 + len(name_label), self.new_task_name, name_field_attr)
                    
                    # Draw date field with proper formatting
                    date_label = "Due Date (YYYY-MM-DD): "
                    stdscr.addstr(height - 3, 2, date_label)
                    date_field_attr = active_field_color if not self.name_input_active else inactive_field_color
                    stdscr.addstr(height - 3, 2 + len(date_label), self.new_task_date, date_field_attr)
                ...
                if self.name_input_active:
                    ...
                    try:
                        name_label = f"{mode} Task Name: "
                        stdscr.move(height - 4, 2 + len(name_label) + len(self.new_task_name))
                    except curses.error:
                        pass
                else:
                    curses.curs_set(1)
                    try:
                        date_label = "Due Date (YYYY-MM-DD): "
                        stdscr.move(height - 3, 2 + len(date_label) + len(self.new_task_date))
                    except curses.error:
                        pass
            else:
                curses.curs_set(0)
            ...
            footer = "q: Quit | a: Add | e: Edit | d: Delete | Space: Toggle | Tab: Category | n: Clear Notifications"
            if self.add_mode or self.edit_mode:
            ...
            if self.add_mode or self.edit_mode:
                ...
                elif key == ord('n'):  # Clear notifications
