...
//            }
    ...
    private void onQuizCompleted(int score) {
        // Save locally
        userScoreRepository.saveUserScore(currentQuiz.getQuizId(), score);
        
        // Update Firebase
        int userId = sessionManager.getUserId();
        int lessonId = currentQuiz.getLessonId();
        
        // Sync progress to Firebase in background
        AppDatabase.databaseWriteExecutor.execute(() -> {
            firebaseDataManager.syncUserProgressAfterQuiz(userId, lessonId, score);
        });
