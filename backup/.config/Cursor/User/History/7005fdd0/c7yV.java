...
//            }
    ...
    private void saveUserScore(int score) {
        try {
            ...
            userScore.setQuizId(currentQuiz.getQuizId());
            userScore.setScore(score);
            userScore.setAttemptDate(getCurrentDate());
            
            // Generate a unique score ID
            userScore.setScoreId(generateUniqueScoreId());
            
            // Save to local database
            long scoreId = userScoreRepository.insertUserScore(userScore);
            
            // Sync to Firebase
            if (scoreId > 0) {
                FirebaseManager.getInstance().saveUserScore(userScore);
            }
            ...
            Log.e(TAG, "Error saving quiz score: " + e.getMessage(), e);
        }
    }
    ...
    private void updateUserProgress(int userId, int score) {
        try {
            // Get lesson ID from the quiz
            int lessonId = currentQuiz.getLessonId();
    ...
    // Helper method to generate a unique score ID
    private int generateUniqueScoreId() {
