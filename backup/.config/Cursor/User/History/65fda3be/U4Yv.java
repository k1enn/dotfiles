...
public class AuthenticationService {
    ...
    public boolean registerUser(String username, String password, boolean isGoogleUser) {
        try {
            ...
            FirebaseUserManager firebaseManager = FirebaseUserManager.getInstance(
                    database.getOpenHelper().getWritableDatabase().getPath().getClass().getContext());

            // First check if username/email exists in Firebase
            CompletableFuture<Boolean> firebaseFuture;
            if (isGoogleUser) {
                firebaseFuture = firebaseManager.checkEmailExistsInFirebase(username);
            } else {
                firebaseFuture = firebaseManager.checkUsernameExistsInFirebase(username);
            }
            
            boolean existsInFirebase = firebaseFuture.get(); // Wait for Firebase check to complete
            
            if (existsInFirebase) {
                Log.d(TAG, "Username/Email already exists in Firebase: " + username);
                return false;
            }
            ...
            boolean userIdExistsInFirebase = firebaseManager.checkUserIdExistsInFirebase(userId).get();
            while (userIdExistsInFirebase) {
                Log.d(TAG, "Generated ID already exists in Firebase, generating a new one");
                userId = generateUniqueId();
                userIdExistsInFirebase = firebaseManager.checkUserIdExistsInFirebase(userId).get();
            }
     ...
     * @return The User object for the currently authenticated user, or null if not logged in
     */
    public User getLocalUser() {
        try {
            ...
            if (user == null) {
                Log.e(TAG, "Session exists but user not found in database: " + userId);
            }
            
            return user;
        } catch (ExecutionException | InterruptedException e) {
            Log.e(TAG, "Error retrieving local user: " + e.getMessage(), e);
            return null;
        }
    }
}