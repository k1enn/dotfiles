                ...
                firebaseManager.syncUserData(user)
                    .thenCompose(success -> {
                        ...
                        if (localProgress == null) {
                            // No local progress found - try to restore from Firebase first using username
                            Log.d(TAG, "No local progress found, attempting to restore from Firebase by username");
                            return firebaseManager.restoreUserProgress(username)
                                .thenCompose(restored -> {
                                    if (restored) {
                                        Log.d(TAG, "Progress restored from Firebase using username");
                                    } else {
                                        Log.d(TAG, "No progress found in Firebase for username, using defaults");
                                    }
                                    // Now sync any changes back to Firebase using username
                                    return firebaseManager.syncUserProgress(username);
                                });
                        } else {
                            // Local progress exists, just sync it to Firebase using username
                            Log.d(TAG, "Local progress found, syncing to Firebase using username");
                            return firebaseManager.syncUserProgress(username);
                        }
                    })
                    .thenAccept(result -> {
                        Log.d(TAG, "User data synchronization completed for: " + username);
                    })
                    .exceptionally(e -> {
