                    ...
                    if (user != null) {
                        ...
                        firebaseManager.syncUserData(user)
                            .thenCompose(success1 -> {
                                // Create a new CompletableFuture for the local progress check
                                CompletableFuture<Boolean> progressCheckFuture = new CompletableFuture<>();
                                ...
                                AppDatabase.databaseWriteExecutor.execute(() -> {
                                    try {
                                        ...
                                        if (localProgress == null) {
                                            // No local progress found - restore from Firebase
                                            firebaseManager.restoreUserProgress(username)
                                                .thenAccept(progressCheckFuture::complete);
                                        } else {
                                            // Local progress exists
                                            progressCheckFuture.complete(true);
                                        }
                                    } catch (Exception e) {
                                        Log.e(TAG, "Error checking local progress", e);
                                        progressCheckFuture.complete(false);
                                    }
                                });
                                
                                return progressCheckFuture;
                            })
