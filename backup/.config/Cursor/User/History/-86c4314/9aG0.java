...
public class MainActivity extends AppCompatActivity {
    ...
    private void setupDictionaryButton() {
        chatbot.setOnClickListener(v -> {
            // If you want to show a quick menu with Dictionary and News options
            PopupMenu popup = new PopupMenu(MainActivity.this, v);
            popup.getMenu().add("Dictionary").setOnMenuItemClickListener(item -> {
                navigationService.navigateToDictionary();
                return true;
            });
            popup.getMenu().add("News").setOnMenuItemClickListener(item -> {
                startActivity(new Intent(MainActivity.this, NewsActivity.class));
                return true;
            });
            popup.show();
    ...
    private void setupNewsButton() {
        ...
        if (navDiscover != null) {
            ...
            // Fallback - add a news button to the menu or existing UI element
            // For example, you could add it to an options menu
            Log.e("MainActivity", "Could not find nav_discover view");
            
            // Alternative: add a menu item for news if nav_discover doesn't exist
            MenuItem item = navigationView.getMenu().findItem(R.id.nav_logout);
            if (item != null && item.getParent() != null) {
                Menu menu = item.getParent();
                MenuItem newsItem = menu.add(Menu.NONE, R.id.nav_news, Menu.NONE, "News");
                newsItem.setIcon(R.drawable.ic_baseline_newspaper_24);
                newsItem.setOnMenuItemClickListener(menuItem -> {
                    startActivity(new Intent(MainActivity.this, NewsActivity.class));
                    return true;
                });
            }
