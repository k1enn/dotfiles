...
public class FirebaseDataManager {
    ...
    /**
     * Pull user progress from Firebase to local database - using DAO directly
     * @param username The username to check and pull data for
     * @param userId The local user ID to update
     * @return CompletableFuture that completes with true if data was pulled successfully
     */
                ...
                public void onDataChange(@NonNull DataSnapshot snapshot) {
                    if (snapshot.exists()) {
                        ...
                        if (userXp != null || userStreak != null) {
                            // Use executor to perform database operation
                            AppDatabase.databaseWriteExecutor.execute(() -> {
                                try {
                                    ...
                                    if (progressList != null && !progressList.isEmpty()) {
                                        // Update each progress entry with the Firebase data
                                        for (UserProgress progress : progressList) {
                                            if (userXp != null) {
                                                progress.setXp(userXp.intValue());
                                            }
                                            
                                            if (userStreak != null) {
                                                progress.setStreak(userStreak.intValue());
                                            }
                                            
                                            // Update directly using DAO
                                            progressRepository.updateUserProgress(progress);
                                        }
                                        
                                        Log.d(TAG, "Updated local data - XP: " + userXp + ", Streak: " + userStreak);
                                        future.complete(true);
                                    } else {
                                        Log.d(TAG, "No progress entries found for user: " + userId);
                                        future.complete(false);
                                    }
                                } catch (Exception e) {
                                    Log.e(TAG, "Error updating progress: " + e.getMessage(), e);
                                    future.complete(false);
                                }
                            });
                        } else {
