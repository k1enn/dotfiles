...
public class FirebaseDataManager {
    ...
    public CompletableFuture<Boolean> pullUserProgressToLocal(String username, int userId) {
        ...
        Log.d(TAG, "Attempting to pull progress for user: " + username);
        
        // First try to get data from leaderboard
        leaderboardRef.child(username)
            .addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
                public void onDataChange(@NonNull DataSnapshot snapshot) {
                    try {
                        if (snapshot.exists()) {
                            Log.d(TAG, "Found user in leaderboard: " + username);
                            
                            // Extract data directly from snapshot to avoid type issues
                            Long userXpLong = snapshot.child("userXp").getValue(Long.class);
                            Long userStreakLong = snapshot.child("userStreak").getValue(Long.class);
                            
                            // Convert to Integer safely
                            Integer firebaseXp = userXpLong != null ? userXpLong.intValue() : null;
                            Integer firebaseStreak = userStreakLong != null ? userStreakLong.intValue() : null;
                            
                            // Debug log
                            Log.d(TAG, "Firebase data - XP: " + firebaseXp + ", Streak: " + firebaseStreak);
                            ...
                            if (localProgress != null && !localProgress.isEmpty()) {
                                Log.d(TAG, "Found local progress entries: " + localProgress.size());
                                ...
                                if (firebaseXp != null && firebaseXp > 0) {
                                    ...
                                    Log.d(TAG, "Successfully updated local progress with Firebase data");
                                    future.complete(true);
                                } else if (firebaseStreak != null) {
                                    // Just update streak if we have it
                                    for (UserProgress progress : localProgress) {
                                        progress.setStreak(firebaseStreak);
                                        progressRepository.updateUserProgress(progress);
                                    }
                                    
                                    Log.d(TAG, "Updated streak only from Firebase");
                                    future.complete(true);
                                } else {
                                    Log.d(TAG, "No valid XP or streak data in Firebase");
                                    future.complete(false);
                                }
                            } else {
                                Log.d(TAG, "No local progress entries found for user: " + userId);
                                future.complete(false);
                            }
                        } else {
                            Log.d(TAG, "User not found in leaderboard: " + username);
                            future.complete(false);
                        }
                    } catch (Exception e) {
                ...
                @Override
                public void onCancelled(@NonNull DatabaseError error) {
                    Log.e(TAG, "Firebase error getting user progress: " + error.getMessage());
                    future.complete(false);
                }
            });
                ...
                public void onDataChange(@NonNull DataSnapshot snapshot) {
                    if (snapshot.exists()) {
                        ...
                        if (storedPassword != null && storedPassword.equals(password)) {
                            ...
                            Boolean isGoogleUser = snapshot.child("isGoogleUser").getValue(Boolean.class);
                            ...
                            // Pull user data to local database
                            syncUserToLocal(user, userXp, userStreak);
