...
public class FirebaseDataManager {
     ...
     * Uses direct approach to avoid nested callbacks that might cause ANR
     */
    public CompletableFuture<Boolean> updateLeaderboard(String username, int userId, int streak, int xp) {
        ...
        // Simplify to a direct update to avoid nesting and potential deadlocks
        Map<String, Object> userUpdate = new HashMap<>();
        userUpdate.put("userId", userId);
        userUpdate.put("userStreak", streak);
        userUpdate.put("userXp", xp);
        
        // Use setValue instead of updateChildren for new users during registration
        // This simplifies the flow and reduces chances of ANR
        leaderboardRef.child(username)
            .setValue(userUpdate)
            .addOnSuccessListener(aVoid -> {
                Log.d(TAG, "Leaderboard entry created/updated successfully for " + username);
                future.complete(true);
            })
            .addOnFailureListener(e -> {
                Log.e(TAG, "Error creating/updating leaderboard entry: " + e.getMessage());
                future.complete(false);
            });
