...
public class FirebaseDataManager {
     ...
     * If user doesn't exist, create new entry with streak=1 and xp=0
     * If user exists, update with provided values
     */
    public CompletableFuture<Boolean> updateLeaderboard(String username, int userId, int streak, int xp) {
        CompletableFuture<Boolean> future = new CompletableFuture<>();
        
        // First check if the username exists in leaderboard
        isUsernameExistsInLeaderboard(username)
            .thenAccept(exists -> {
                ...
                if (!exists) {
                    Log.d(TAG, "Creating new leaderboard entry for " + username);
                    userUpdate.put("userStreak", 1);  // Initial streak for new users
                    userUpdate.put("userXp", 0);      // Initial XP for new users
                } else {
                    Log.d(TAG, "Updating existing leaderboard entry for " + username);
                ...
                leaderboardRef.child(username)
                    ...
                    .addOnSuccessListener(aVoid -> {
                        Log.d(TAG, "Leaderboard " + (exists ? "updated" : "created") + 
                              " successfully for " + username);
                        future.complete(true);
                    })
                    .addOnFailureListener(e -> {
                        Log.e(TAG, "Error " + (exists ? "updating" : "creating") + 
                              " leaderboard entry: " + e.getMessage());
                        future.complete(false);
                    });
            })
            .exceptionally(e -> {
                Log.e(TAG, "Error checking leaderboard existence: " + e.getMessage());
                future.complete(false);
                return null;
            });
