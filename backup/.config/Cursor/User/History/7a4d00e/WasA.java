    ...
    private int getTotalUserXp(int userId) {
        ...
        try {
            ...
            if (progressList != null && !progressList.isEmpty()) {
                ...
                // Create a map to track lesson IDs we've already processed
                Set<Integer> processedLessonIds = new HashSet<>();
                
                // Sum up XP from all progress entries (only once per lesson)
                for (UserProgress progress : progressList) {
                    if (progress != null) {
                        int lessonId = progress.getLessonId();
                        
                        // Only count each lesson once
                        if (!processedLessonIds.contains(lessonId)) {
                            processedLessonIds.add(lessonId);
                            totalXp += progress.getXp();
                            Log.d(TAG, "Adding XP from lesson " + lessonId + ": " + progress.getXp() + 
                                  ", running total: " + totalXp);
                        } else {
                            Log.d(TAG, "Skipping duplicate lesson " + lessonId);
                        }
                    }
                }
                Log.d(TAG, "Final total XP for user " + userId + ": " + totalXp);
            } else {
