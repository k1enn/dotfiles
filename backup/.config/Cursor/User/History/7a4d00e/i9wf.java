    ...
    private void updateUserProgressWithFirebaseData(int userId, int xp, int streak) {
        AppDatabase.databaseWriteExecutor.execute(() -> {
            try {
                ...
                // Kiểm tra xem có bài học nào trong database không
                List<Lesson> lessons = lessonRepository.getAllLessons();
                if (lessons == null || lessons.isEmpty()) {
                    Log.e(TAG, "Cannot update progress: No lessons found in local database");
                    return;
                }
                
                // Lấy ID của bài học đầu tiên
                ...
                if (progressList != null && !progressList.isEmpty()) {
                    ...
                    newProgress.setUserId(userId);
                    newProgress.setLessonId(firstLessonId); // Sử dụng ID bài học đầu tiên
