                                    ...
                                    if (progressId != null && lessonId != null) {
                                        ...
                                        if (existingProgress != null) {
                                            ...
                                            if (completionTime != null)
                                                existingProgress.setCompletionTime(completionTime);
...
//                                // Also update user table
    ...
    private int getTotalUserXp(int userId) {
        ...
        try {
            UserProgressDao userProgressDao = progressRepository.getUserProgressDao();
            
            // APPROACH 1: Get total XP using SQL SUM
            // This is the most reliable method as it lets the database handle the calculation
            Integer summedXp = userProgressDao.getTotalXpForUser(userId);
            if (summedXp != null) {
                totalXp = summedXp;
                Log.d(TAG, "Using database SUM: Total XP for user " + userId + ": " + totalXp);
                return totalXp;
            }
            
            // APPROACH 2: If the above fails, use manual calculation with a detailed debug
            Map<Integer, Integer> lessonXpMap = new HashMap<>();
            
            // Get all progress entries for this user
            List<UserProgress> progressList = userProgressDao.getUserProgress(userId);
            
            if (progressList != null && !progressList.isEmpty()) {
                Log.d(TAG, "Found " + progressList.size() + " progress entries for user " + userId);
                
                // Map each lesson to its XP (keeping only the highest value for each lesson)
                for (UserProgress progress : progressList) {
                    if (progress != null) {
                        int lessonId = progress.getLessonId();
                        int xp = progress.getXp();
                        
                        Log.d(TAG, "Processing entry: LessonID=" + lessonId + 
                              ", ProgressID=" + progress.getProgressId() + 
                              ", XP=" + xp);
                        
                        // If we already have an entry for this lesson, only keep the highest XP
                        if (lessonXpMap.containsKey(lessonId)) {
                            int existingXp = lessonXpMap.get(lessonId);
                            if (xp > existingXp) {
                                Log.d(TAG, "Updating XP for lesson " + lessonId + " from " + existingXp + " to " + xp);
                                lessonXpMap.put(lessonId, xp);
                            } else {
                                Log.d(TAG, "Keeping existing higher XP " + existingXp + " for lesson " + lessonId);
                            }
                        } else {
                            Log.d(TAG, "Adding new lesson " + lessonId + " with XP " + xp);
                            lessonXpMap.put(lessonId, xp);
                        }
                    }
                }
                
                // Sum up all the highest XP values
                for (Map.Entry<Integer, Integer> entry : lessonXpMap.entrySet()) {
                    totalXp += entry.getValue();
                    Log.d(TAG, "Adding to total: Lesson " + entry.getKey() + " contributes " + entry.getValue() + " XP");
                }
                
                Log.d(TAG, "Manual calculation: Total XP for user " + userId + ": " + totalXp);
            } else {
                Log.d(TAG, "Cannot find any progress for user: " + userId);
            }
        } catch (Exception e) {
            Log.e(TAG, "Error calculating total XP: " + e.getMessage(), e);
        }
