...
public class FirebaseDbHelper {
     ...
     * @return CompletableFuture with success status
     */
    public CompletableFuture<Boolean> saveVocabulary(Vocabulary vocabulary) {
        CompletableFuture<Boolean> future = new CompletableFuture<>();
        
        try {
            Map<String, Object> vocabValues = new HashMap<>();
            vocabValues.put("vocabularyId", vocabulary.getVocabularyId());
            vocabValues.put("lessonId", vocabulary.getLessonId());
            vocabValues.put("word", vocabulary.getWord());
            vocabValues.put("definition", vocabulary.getDefinition());
            vocabValues.put("example", vocabulary.getExample());
            
            rootRef.child(VOCABULARY_PATH).child(String.valueOf(vocabulary.getVocabularyId()))
                   .updateChildren(vocabValues)
                   .addOnSuccessListener(aVoid -> {
                       Log.d(TAG, "Vocabulary saved successfully: " + vocabulary.getVocabularyId());
                       future.complete(true);
                   })
                   .addOnFailureListener(e -> {
                       Log.e(TAG, "Error saving vocabulary: " + e.getMessage(), e);
                       future.complete(false);
                   });
        } catch (Exception e) {
            Log.e(TAG, "Error preparing vocabulary data: " + e.getMessage(), e);
            future.complete(false);
        }
    ...
    /**
     * Save multiple vocabulary items to Firebase
     * 
     * @param vocabularyList List of vocabulary items to save
     * @return CompletableFuture with success status
     */
    public CompletableFuture<Boolean> saveVocabularyList(List<Vocabulary> vocabularyList) {
        ...
        try {
            ...
            for (Vocabulary vocabulary : vocabularyList) {
                ...
                updates.put(String.valueOf(vocabulary.getVocabularyId()), vocabValues);
            }
            
            rootRef.child(VOCABULARY_PATH)
                   .updateChildren(updates)
                   .addOnSuccessListener(aVoid -> {
                       Log.d(TAG, "Saved " + vocabularyList.size() + " vocabulary items successfully");
                       future.complete(true);
                   })
                   .addOnFailureListener(e -> {
                       Log.e(TAG, "Error saving vocabulary list: " + e.getMessage(), e);
                       future.complete(false);
                   });
        } catch (Exception e) {
            Log.e(TAG, "Error preparing vocabulary list data: " + e.getMessage(), e);
            future.complete(false);
        }
                ...
                try {
                    for (DataSnapshot vocabSnapshot : dataSnapshot.getChildren()) {
                        try {
                            ...
                            String definition = vocabSnapshot.child("definition").getValue(String.class);
                            String example = vocabSnapshot.child("example").getValue(String.class);
                            
                            Vocabulary vocabulary = new Vocabulary(vocabularyId, lessonId, word, definition, example);
