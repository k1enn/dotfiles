...
public class FirebaseDbHelper {
    ...
    /**
     * Save a list of vocabulary items to Firebase
     * 
     * @param vocabularyList The list of vocabulary items to save
     * @return CompletableFuture with success status
     */
    public CompletableFuture<Boolean> saveVocabularyList(List<Vocabulary> vocabularyList) {
        CompletableFuture<Boolean> future = new CompletableFuture<>();
        
        try {
            if (vocabularyList == null || vocabularyList.isEmpty()) {
                Log.w(TAG, "Empty vocabulary list provided to saveVocabularyList");
                future.complete(true); // Return true since there's nothing to save
                return future;
            }
            
            // Create a batched update map
            Map<String, Object> updates = new HashMap<>();
            ...
            rootRef.child(VOCABULARY_PATH).updateChildren(updates)
                   .addOnSuccessListener(aVoid -> {
                       Log.d(TAG, "Successfully saved " + vocabularyList.size() + " vocabulary items to Firebase");
                       future.complete(true);
                   })
                   .addOnFailureListener(e -> {
                       Log.e(TAG, "Error saving vocabulary list to Firebase: " + e.getMessage(), e);
                       future.complete(false);
                   });
        } catch (Exception e) {
            Log.e(TAG, "Exception in saveVocabularyList: " + e.getMessage(), e);
            future.complete(false);
        }
    ...
    /**
     * Save a list of lessons to Firebase
     * 
     * @param lessonList The list of lessons to save
     * @return CompletableFuture with success status
     */
    public CompletableFuture<Boolean> saveLessons(List<Lesson> lessonList) {
        CompletableFuture<Boolean> future = new CompletableFuture<>();
        
        try {
            if (lessonList == null || lessonList.isEmpty()) {
                Log.w(TAG, "Empty lesson list provided to saveLessons");
                future.complete(true); // Return true since there's nothing to save
                return future;
            }
            ...
            for (Lesson lesson : lessonList) {
                String key = String.valueOf(lesson.getLessonId());
                ...
                updates.put(key, lessonValues);
            }
            
            // Batch update to Firebase
            rootRef.child(LESSONS_PATH).updateChildren(updates)
                   .addOnSuccessListener(aVoid -> {
                       Log.d(TAG, "Successfully saved " + lessonList.size() + " lessons to Firebase");
                       future.complete(true);
                   })
                   .addOnFailureListener(e -> {
                       Log.e(TAG, "Error saving lessons to Firebase: " + e.getMessage(), e);
                       future.complete(false);
                   });
        } catch (Exception e) {
            Log.e(TAG, "Exception in saveLessons: " + e.getMessage(), e);
            future.complete(false);
        }
    ...
    /**
     * Save a single lesson to Firebase
     * 
    ...
    public CompletableFuture<Boolean> saveLesson(Lesson lesson) {
        ...
        try {
            if (lesson == null) {
                Log.w(TAG, "Null lesson provided to saveLesson");
                future.complete(false);
                return future;
            }
     ...
     * @return CompletableFuture with the user or null if not found
     */
    public CompletableFuture<User> getUserByEmail(String email) {
        CompletableFuture<User> future = new CompletableFuture<>();
        
        if (email == null || email.isEmpty()) {
            Log.w(TAG, "Empty email provided to getUserByEmail");
            future.complete(null);
            return future;
        }
        
        rootRef.child(USERS_PATH).orderByChild("gmail").equalTo(email)
               .limitToFirst(1)
               .addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                try {
                    if (!dataSnapshot.exists() || !dataSnapshot.hasChildren()) {
                        Log.d(TAG, "User with email " + email + " not found in Firebase");
                        future.complete(null);
                        return;
                    }
                    ...
                    Log.d(TAG, "Successfully fetched user with email " + email);
                    future.complete(user);
                } catch (Exception e) {
                    Log.e(TAG, "Error parsing user data: " + e.getMessage(), e);
                    future.complete(null);
            ...
            @Override
            public void onCancelled(@NonNull DatabaseError databaseError) {
                Log.e(TAG, "Error fetching user by email: " + databaseError.getMessage());
                future.complete(null);
            }
        });
    ...
    /**
     * Fetch user stats (XP and streak) from Firebase
     * 
     * @param userId The user ID
     * @return CompletableFuture with UserProgress object or null if not found
     */
    public CompletableFuture<UserProgress> getUserStats(int userId) {
        CompletableFuture<UserProgress> future = new CompletableFuture<>();
        ...
        rootRef.child(USERS_PATH).child(String.valueOf(userId))
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                try {
                    ...
                    // Extract XP and streak from user data
                    Integer xp = dataSnapshot.child("xp").getValue(Integer.class);
                    Integer streak = dataSnapshot.child("streak").getValue(Integer.class);
                    ...
                    progress.setXp(xp != null ? xp : 0);
                    progress.setStreak(streak != null ? streak : 0);
                    
                    Log.d(TAG, "Successfully fetched stats for user " + userId);
                    future.complete(progress);
                } catch (Exception e) {
                    Log.e(TAG, "Error parsing user stats: " + e.getMessage(), e);
                    future.complete(null);
                }
            }
            ...
            @Override
            public void onCancelled(@NonNull DatabaseError databaseError) {
                Log.e(TAG, "Error fetching user stats: " + databaseError.getMessage());
                future.complete(null);
            }
        });
    ...
    /**
     * Save user data with progress information to Firebase
     */
    public CompletableFuture<Boolean> saveUserWithProgress(User user, UserProgress progress) {
        CompletableFuture<Boolean> future = new CompletableFuture<>();
        ...
        try {
            ...
            // Add progress data to the same record
            if (progress != null) {
                userData.put("xp", progress.getXp());
                userData.put("streak", progress.getStreak());
                userData.put("lastStudyDate", progress.getLastStudyDate());
                userData.put("lessonsCompleted", progress.getLessonId() > 0 ? 1 : 0);
            } else {
                userData.put("xp", 0);
                userData.put("streak", 0);
                userData.put("lastStudyDate", "");
                userData.put("lessonsCompleted", 0);
            }
            ...
            rootRef.child(USERS_PATH).child(String.valueOf(user.getUserId()))
                   ...
                   .addOnSuccessListener(aVoid -> {
                       Log.d(TAG, "User with progress saved successfully: " + user.getUserId());
                       future.complete(true);
                   })
                   .addOnFailureListener(e -> {
                       Log.e(TAG, "Error saving user with progress: " + e.getMessage(), e);
                       future.complete(false);
                   });
        } catch (Exception e) {
            Log.e(TAG, "Error preparing user data: " + e.getMessage(), e);
