...
class _CryptoDetailScreenState extends State<CryptoDetailScreen> {
      ...
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              _buildHeader() as Widget,
              const SizedBox(height: 24),
              _buildTimeRangeSelector() as Widget,
              const SizedBox(height: 24),
              _buildPriceChart() as Widget,
              const SizedBox(height: 24),
              _buildPriceStatistics() as Widget,
            ],
          ),
        ),
      ),
  ...
  PreferredSizeWidget _buildAppBar() {
    return AppBar(
      ...
      elevation: 0,
      //TODO: Need to change this button
      leading: IconButton(
        onPressed: () => Navigator.of(context).pop(),
        icon: const Icon(Icons.arrow_back),
      ),
      title: Text(widget.crypto.name, style: AppTextStyles.heading2),
      centerTitle: true,
    );
  }

  NeumorphicContainer _buildHeader() {
  ...
  NeumorphicContainer _buildTimeRangeSelector() {
  ...
  NeumorphicContainer _buildPriceChart() {
    return NeumorphicContainer(
      padding: const EdgeInsets.all(16),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Price Chart ($_timeRange)', style: AppTextStyles.heading3),
          const SizedBox(height: 16),
          CryptoLineChart(crypto: widget.crypto, height: 250),
        ],
      ),
    );
  }

  NeumorphicContainer _buildPriceStatistics() {
    return NeumorphicContainer(
      padding: const EdgeInsets.all(16),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          _buildStatisticRow(
            'Market Cap',
            '\$${(widget.crypto.price * 19000000).toStringAsFixed(0)}',
          ),
          _buildStatisticRow(
            'Volume (24h)',
            '\$${(widget.crypto.price * 500000).toStringAsFixed(0)}',
          ),
          _buildStatisticRow(
            'Circulating Supply',
            '${widget.crypto.symbol} ${(19000000).toString()}',
          ),
          _buildStatisticRow(
            'Total Supply',
            '${widget.crypto.symbol} ${(21000000).toString()}',
          ),
